...
class TopicListService {
  ...
  public static function createAbstract($post_content, $text_length = 30) {
    ...
    if (!is_null($offset)) {
      $text = mb_substr(trim(strip_tags(mb_substr(trim($post_content), $offset))), 0, $text_length, 'UTF-8');
    }
  ...
  private static function parseHeadline($post_content, &$offset) {
    ...
    if ($post_content[0] !== '<') {
      ...
      $end = mb_strpos($post_content, '<');
      if ($end === false) {
        return $post_content;
      } else {
        $offset = $end;
        return mb_substr($post_content, 0, $end);
      }
    } else {
      $i = mb_strpos($post_content, ' '); // Search the first whitespace
      $j = mb_strpos($post_content, '>');
      ...
      $tag = mb_substr($post_content, 1, $tagNameEnd - 1);
      $tagEnd = mb_strpos($post_content, '>');
      if ($tagEnd === false) {
        # What? No idea Oo
        return $post_content;
      }

      $end = mb_strpos($post_content, '</' . $tag . '>', $tagEnd);
      if ($end === false) {
        # Srsly?
        return $post_content;
      }

      $offset = $end + 3 + mb_strlen($tag);
      return mb_substr($post_content, $tagEnd + 1, $end - $tagEnd);
    }
  }
}